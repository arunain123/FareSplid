...
def add_personal_budget(request):
    if request.method == 'POST':
        ...
        try:
            if (not str(amount).isdigit() or amount==None  or int(amount)<=0):
                messages.warning(request,"Please enter valid amount!!")
            ...
            if (type == 'inc'):
                ...
                qs_inc = Personal_income.objects.filter(user = request.user)
                total_inc = sum([x.amount for x in qs_inc])
                if (total_inc < int(amount)):
                    messages.error(request,"Your Balance is not Sufficient")
                    return HttpResponseRedirect(reverse('splitter:personal_budget'))
...
def send_remainder(request, username, amount):
    reciever = User.objects.filter(username=username)
    user = request.user

    # print(user, reciever.only().get('email'))
    email = ''
    for rc in reciever:
        email = rc.email
        break
    
    res = send_email(email, amount, user)
    if (res):
        messages.success(request,"Remainder had been sent!!!")
    else:
        messages.error(request,"Error!! Remainder had not been sent!!!")
    return HttpResponseRedirect(reverse('splitter:my_debts'))
...
def send_email(email, amount, curr_user):
    user = "guptaproject4@gmail.com"
    password = "mgplkbautsetwkei"
    try:
